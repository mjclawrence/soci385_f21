...
2. What would you expect about the relationship between income and life expectancy in countries? Try a plot first (with `gross_national_income` as the x and `life_expectancy` as the y. 
...
```{r}
income_log_life_expectancy_plot <- ggplot(hdi, aes(x = log_gross_national_income,
                                 y = life_expectancy)) + geom_point() +
  geom_smooth(method = lm)
...
Now run the regression model on the transformed income variable.
...
### REPLACE THIS LINE WITH YOUR CODE ###
```{r}
exp(hdi$log_gross_national_income[hdi$country=="United Kingdom"])
hdi$log_gross_national_income[hdi$country=="United Kingdom"]
19.5244 + (5.6811*10.54352)
```
