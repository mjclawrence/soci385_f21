...
We can pull samples from the values of age any number of times. In this example, we'll see what happens if we pull 1000 samples with 2500 observations in each sample.
...
We can write a new function to allow for more comparisons.
...
Now we recall the name of the function and provide two values: the number of pulls and the size of each pull. In this example, we'll have 100 samples each with 50 observations. Are the means normally distributed now?

```{r}
sampling_function(300, 500)
```
...
One efficient way would be to use the `str_detect()` function, which *detects* if character *strings* match a pattern. In this example, we'll use `mutate()` to create a new binary variable distinguishing those respondents with kids (1) from those without (0).
