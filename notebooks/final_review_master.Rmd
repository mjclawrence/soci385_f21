...
huxtable(space_summary) |> 
  ...
  set_align(everywhere, 3:5, "center") |> 
  theme_compact() |> 
  set_bold(row = 4, col = 3) |> # Bold the cell in row 4 and column 2
  set_italic(row = 5, col = everywhere) |> # Italicize all cells in row 5
  set_background_color(row = -(1:2), col = 5, "grey") # Change the background color to grey for all cells but the header rows in column 5; lots of other colors work too
```
