...
```{r}
library(pollster)
review <- mutate(review, weight = 1)
pollster_examples <- crosstab_3way(review, religion, space, science, pct_type = "row", weight = weight)
...
```{r}
space_summary |> 
  pivot_longer(names_to = "space", values_to = "prop", 3:5) |> 
  mutate(space = factor(space, levels = c("not_interested", "moderately_interested", "very_interested"))) |> 
  ggplot(aes(x = religion, y = prop, fill = space)) + geom_col() + theme(legend.position = "bottom")
```
