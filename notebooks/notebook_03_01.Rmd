# Setting Up
This week we will continue using the 2008-2018 waves of the General Social Survey. We are still focused on the `agekdbrn` variable but will start looking at how its distribution varies by education and race/ethnicity. Start by loading tidyverse and the data. 

```{r load packages and data, echo = FALSE, warning = FALSE, message = FALSE}
library(tidyverse)

gss_week3 <- read.csv("gss_week3.csv") # A path in your project folder
```
