...
On Monday, we ended class by creating density plots, bar plots, and histograms to visualize the distribution of age at first birth and years of education. To warm up, make a bar plot of the racial and ethnic composition of our sample (use the `racehisp` variable).

```{r}
racehisp_plot <- ggplot(gss_week3,
                        aes(x = racehisp))

racehisp_plot + geom_bar() + labs(title = "Distribution of Race or Ethnicity")
```
...
educ_barplot <- ggplot(gss_week3, aes(x = racehisp))

educ_barplot + geom_bar() + 
     labs(x = "Race or Ethnicity",
          y = "Count",
          title = "Distribution of Respondents' Race or Ethnicity",
...
"Create a new variable called `college_degree`. If the value of the education variable is greater than or equal to 16, the new variable's value should be 1. If the value of the education variable is anything else, the new variable's value should be 0."

Then we assert that it is a factor variable with the labels No (for values of 0) and Yes (for values of 1).
...
There are a couple other `dplyr` functions that help with finding values of one variable for all values of another variable. For example, we might want to find the mean age at first birth for each race/ethnicity category. To do so, we first group all the observations by the categorical variable. Then we create a variable that summarizes one variable for all the categories of our other variable. And we can connect these functions with pipes. We'll save the output in a new dataframe named `agekdbrn_summary`.

```{r group by and summarize means}
agekdbrn_summary <- gss_week3 |> 
     group_by(racehisp) |>  # We want info about each level of this variable
     summarize(mean_agekdbrn = round(mean(agekdbrn),2)) # The info we want

agekdbrn_summary
```

This `summarize()` function works with a lot of the basic commands we have discussed (min, max, sd, var, etc.). Try finding the median age at first birth for each education category. You can still use the `agekdbrn_summary` object:
