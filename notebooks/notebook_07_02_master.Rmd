...
For another example, let's look at 2018's mean ideal number of children (variable = `chldidel`). Using `t.test()`, can we reject the null hypothesis that the 2016 mean is the same as 2.55 at the .05 alpha level?
...
Can we reject the null hypothesis that the 2016 mean is the same as 2.55 at the .01 alpha level? We can change the default level of .05 to .01 using the conf.level option (which requires the confidence level, so .99 for the .01 alpha level).

### REPLACE THIS LINE WITH YOUR CODE

```{r t test 2 in r exercise}
t.test(gss_week7$chldidel[gss_week7$year==2014], mu = 2.63,
       conf.level = .99)
```
