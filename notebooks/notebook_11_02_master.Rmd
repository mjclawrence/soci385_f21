...
We'll use the `midd_survey.csv` file today. These are selected responses to a survey of Middlebury students conducted by Professor Peggy Nelson and her research methods class in Fall 2011. Load it as a data frame called `midd_survey` and load the usual packages.
...
```{r}
huxtable(lookrel_predictions) |> 
  set_contents(1, 1:3, c("Gender", "Year", "Agree Looking For Relationship")) |> 
...
```{r}
midd_survey$pred_newpeople <- newpeople_raceXclass_model$fitted.values
