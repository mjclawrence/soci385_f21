...
You can test if `tinytex` was installed correctly by opening `R Studio` again. Click the `File` menu bar, scroll down to `New File`, and select `R Markdown`. If a message appears suggesting you install additional packages, do so. Enter your name in the `Author` field,  enter "Assignment One" in the `Title` field, select "PDF" as the `Default Output Format`, and then click `OK` . A default template should open in the top left panel of `R Studio`.
