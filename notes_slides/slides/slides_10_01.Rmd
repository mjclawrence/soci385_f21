...
### 1. Without using any R shortcuts, find the 95% confidence interval for the mean of `eqwlth` in each year.
...
labs(x = "Year",
     ...
     subtitle = "General Social Survey, 2010 - 2016",
...
### 2. Which (if any) age categories showed significant differences in mean `eqwlth` scores between the 2014 and 2016 surveys?
...
multiple_ttests <- ps2 %>%
     ...
     summarise_each(list(~t.test(.[year == 2014],
                                 .[year == 2016])$statistic,
                         ~t.test(.[year == 2014],
                                 .[year == 2016])$p.value), 
                    vars = eqwlth)
...
hdi$predicted_life_expectancy <- 
    fitted.values(schooling_life_model1)
```
...
### Regress the maternal mortality ratio on the percent of females with some secondary education
...
### An increase of one point in the percentage of females with some secondary education is associated with a decrease in the maternal mortality ratio of 5.25.